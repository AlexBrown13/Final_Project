...
html[data-persona="researcher"]       { font-size: 13px; }
```
...
(DYNAMIC_QUESTION_SYSTEM_PROMPT — now probes for primary_topic)
        ↓
Quiz completes → Groq classifies persona
(PERSONA_PROFILE_SYSTEM_PROMPT — now returns 5 fields including primary_topic)
        ↓
