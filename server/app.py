...
SYSTEM_PROMPT = """
אתה מנתח תשובות של משתמש ומסווג אותו לפי רמת ידע והתעניינות בנושא טראומה בישראל:

1. 1 - מתחיל: המשתמש רוצה לדעת על טראומה אבל אין לו ידע בפסיכולוגיה
2. 2 - בינוני: המשתמש סקרן ורוצה להבין גם מעט על מחקרים וסטטיסטיקות
3. 3 - חוקר: המשתמש מבין בפסיכולוגיה, רוצה נתונים, מחקרים וסטטיסטיקות
...
QUESTIONS = [
    "מה הביא אותך לכאן היום? מה אתה רוצה לדעת על טראומה?",
    "עד כמה אתה מתעניין בנתונים או מחקרים על טראומה בישראל?",
    "האם יש לך רקע בפסיכולוגיה או נושאים קשורים?",
    "איך אתה אוהב ללמוד: סיפורים, עובדות, מחקרים או סטטיסטיקות?",
    "כמה חשוב לך להבין את עומק הבעיה וההשלכות שלה על אנשים?"
]
    ...
    messages=[
        {
            "role": "system",
            "content": (
                "אתה מנהל שיחה עם משתמש כדי להבין את רמת הידע והעניין שלו בנושא טראומה בישראל. "
                "המטרה שלך היא לשאול שאלות המשך באופן טבעי, כדי לגלות עד כמה המשתמש מבין בפסיכולוגיה, "
                "כמה הוא סקרן לגבי נתונים וסטטיסטיקות, ומה רמת העניין שלו במחקרים. "
                "חוקים חשובים:"
                "- התייחס ספציפית לתשובה האחרונה של המשתמש"
                "- שאל שאלה אחת בלבד בכל פעם"
                "- כתוב בצורה טבעית, ידידותית ולא רשמית מדי"
                "- שאל שאלה שמסייעת להעריך את המשתמש בין 1 (מתחיל) ל-3 (חוקר)"
            )
        },
        {
            "role": "user",
            "content": (
                ...
                "שאל שאלה המשך טבעית שמתייחסת למה שהמשתמש אמר, שתעזור להעריך את רמת הידע/התעניינות שלו."
            )
        }
    ]
)
